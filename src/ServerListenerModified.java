...
public class ServerListenerModified implements Runnable
{
    ...
    public ServerListenerModified(ObjectInputStream is, ObjectOutputStream os, char player) {
        this.is = is;
        this.os = os;
        this.player = player;
        outs.add(os);
    ...
    public void changeTurn()
    {
        ...
        if (turn == 'R')
            ...
            sendCommand(new CommandFromServer(CommandFromServer.BLUE_TURN, null));
    }

    public void checkGameOver()
    {
        //isRedTurn && winner = red win
        int command = -1;
        if(gameData.isCat())
            command = CommandFromServer.TIE;
        else if(gameData.isWinner() && player=='B')
            command = CommandFromServer.RED_TURN;
        else if(gameData.isWinner() && player=='R' )
            command = CommandFromServer.BLUE_TURN;
